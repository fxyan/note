...
IP协议:
    ...
    ARP欺骗
        B返回一个我是的信息，然后本来不是的C返回一个我是的消息，覆盖了B的消息所以A就傻傻的将数据传给了B这就是
        ARP欺骗
    MAC地址:
        MAC和IP的存在关系
...
UDP:
   ...
   标志位:
        PSH 如果置1那么这个数据被接收方接受的时候，会直接到缓存队列的最前面被接收
        URG 如果置1那么这个数据不会到滑动窗口中排队，会直接插队发送
   ...
   三次握手
    ...
    三次握手:
        SYN_SENT
            ...
            等到接收到服务器返回的确认号之前处于这个阶段，之后发送确认号建立连接
    ...
    滑动窗口:
        ...
        接收窗口
            对面在建立连接的时候会告诉对面
        窗口的大小是取上面两个窗口的最小值
        在建立连接的时候双方会交换两边的窗口大小也就是缓存大小(也就是你可以发送多少字节，)
        发送缓存一般就是这个缓存的大小，接收方还会根据这个创建一个接收缓存来接受发送方传来的数据。
        ...
        可用窗口3类
    流量控制:
        在进行链接的时候还会互相交换一些细节，MSS最大接收数据包，两边在建立连接的时候都会互相交换这个数据，表明自身能够接收到
        的最大数据包，这样发送方就会按照这个来控制自己发送的的数据长度。
        也就是窗口大小(也就是缓冲区的大小)，如果接收端的缓冲区已经满了可能会溢出没这样就会再告诉发送端一个更小的值防止浪费资源，
        这个值在报文的首部，是一个可以随时变动的值(缓冲区变小就减小，缓冲区变大就增加)，通过这个来进行TCP流量控制。
    拥塞阻塞机制
        慢开始与拥塞避免:
            设置一个cwnd值每次收到这个确认信息就加倍这个值(这个值为发送报文段的数量)一般设为1
            因为这个值增长的太快所以设定一个门限值ssthresh 当cwnd值比门限值大的时候就每次只增加一个报文段
            如果出现超时那么令ssthresh的值为cwnd/2重新执行慢开始
        快重传和快恢复
            快重传 
                如果发送了1 2 3 4 5 这几个数据包，然后发现3丢了而4接收到了，这个时候返回3个ack 3 的确认号发送端接收到
                就会重发3的数据包
            快恢复:
                和快重传配套的算法ssthresh的计算和慢开始相同与上面的慢开始不同的是，从新开始传输的值并不是从1开始，
                而是直接和ssthresh值相同，进行+1+1的增长。
UDP:
    ...
    首部 20个字节
    伪首部 12个字节
        源IP地址 目标IP地址 还有但是我不想写了
    首部
        源端口号 目的端口号 长度 校验和
    校验和根据这一系列的东西进行计算来判断是不是被损坏了
"""